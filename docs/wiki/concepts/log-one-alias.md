...
_See textbook excerpt below or [`tasks/log-one-alias/`](../../../tasks/log-one-alias/) if available._

## Bach's textbook treatment

# Bach textbook excerpt — log 1 = 0 (alias)

**Concept ID:** `log-one-alias`
**Chapter:** Ch 1
...
- Bach uses log 1 = 0 implicitly when normalizing cumulant generating functions phi(0) = log E[1] = log 1 = 0 in the Hoeffding lemma proof (page 10).
...
- Lean target: forward the corresponding Mathlib lemma (`Real.log_one`).
...
- **Theorem/def name:** `log_one_alias`
- **Status:** (unaudited)
