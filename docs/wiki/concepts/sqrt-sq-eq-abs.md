...
_See textbook excerpt below or [`tasks/sqrt-sq-eq-abs/`](../../../tasks/sqrt-sq-eq-abs/) if available._

## Bach's textbook treatment

# Bach textbook excerpt — sqrt(x²) = |x| (alias)

**Concept ID:** `sqrt-sq-eq-abs`
**Chapter:** Ch 1
...
(Mathlib lemma: `Real.sqrt_sq_eq_abs`.)
...
- Bach uses square-root identities implicitly when stating sub-Gaussian deviation bounds, e.g., the √(2τ² log n) bound in Proposition 1.5 (page 16). Standard real-analysis prerequisite.
...
- Lean target: forward the corresponding Mathlib lemma (`Real.sqrt_sq_eq_abs`).
...
- **Theorem/def name:** `sqrt_sq_eq_abs`
- **Status:** (unaudited)
