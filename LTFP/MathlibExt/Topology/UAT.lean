...
/-- **Discharge of Tier C universal-approximation row.** For any
compact space `X` admitting an injective continuous map
...
element of this subalgebra. This theorem itself does *not* assert
that the approximating `g` is in the image of `singleHiddenRampNN`;
it is the Stone–Weierstrass density statement for the
ramp-generated subalgebra. Combined with the bridge lemma, it
exhibits the class of explicit single-hidden-layer ramp networks as
a dense subset (in fact, a `Submodule`-dense one — a sum-of-scalar-
ramps expression) of `C(X, ℝ)`, recovering Cybenko (1989) / Hornik
(1991). -/
theorem cybenko_uat_singleHiddenRampNN
    {ι : C(X, ℝ)} (hι : Function.Injective ι)
    (f : C(X, ℝ)) {ε : ℝ} (hε : 0 < ε) :
...
set_option linter.unusedSectionVars false in
/-- Pointwise / unbundled version of `cybenko_uat_singleHiddenRampNN`:
given a continuous target `f : X → ℝ` on a compact space `X` with an
...
As with `cybenko_uat_singleHiddenRampNN`, the approximant `g` is
asserted only to lie in `rampSubalgebra ι`; the bridge lemma
...
subalgebra. -/
theorem cybenko_uat_singleHiddenRampNN_pointwise
    {ι : C(X, ℝ)} (hι : Function.Injective ι)
    (f : X → ℝ) (hf : Continuous f) {ε : ℝ} (hε : 0 < ε) :
    ∃ g : C(X, ℝ), g ∈ rampSubalgebra ι ∧ ∀ x, ‖g x - f x‖ < ε :=
